...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'DefaultIndicatorList'
        db.create_table('indicators_defaultindicatorlist', (
        ...
        db.send_create_signal('indicators', ['DefaultIndicatorList'])

        # Adding M2M table for field indicators on 'DefaultIndicatorList'
        db.create_table('indicators_defaultindicatorlist_indicators', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('defaultindicatorlist', models.ForeignKey(orm['indicators.defaultindicatorlist'], null=False)),
            ('indicator', models.ForeignKey(orm['indicators.indicator'], null=False))
        ))
        db.create_unique('indicators_defaultindicatorlist_indicators', ['defaultindicatorlist_id', 'indicator_id'])

        # Adding unique constraint on 'TypeIndicatorLookup', fields ['name']
        db.create_unique('indicators_typeindicatorlookup', ['name'])

        # Adding unique constraint on 'SubDataSourceDisclaimer', fields ['name']
        db.create_unique('indicators_subdatasourcedisclaimer', ['name'])


    def backwards(self, orm):
        # Removing unique constraint on 'SubDataSourceDisclaimer', fields ['name']
        db.delete_unique('indicators_subdatasourcedisclaimer', ['name'])

        # Removing unique constraint on 'TypeIndicatorLookup', fields ['name']
        db.delete_unique('indicators_typeindicatorlookup', ['name'])

        # Deleting model 'DefaultIndicatorList'
        db.delete_table('indicators_defaultindicatorlist')

        # Removing M2M table for field indicators on 'DefaultIndicatorList'
        db.delete_table('indicators_defaultindicatorlist_indicators')
