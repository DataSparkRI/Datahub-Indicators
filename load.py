...
def open_xsl():
    ...
    return xlrd.open_workbook(os.path.join(_directory,crosswalk_file))
...
def get_metadata():
    ...
    if not _metadata:
        ...
        for row_num in range(1, sheet.nrows):
            ...
            _metadata.append(metadata_dict)
        return _metadata
...
def get_files():
    ...
    global _directory
    files = {}
    for path, subpaths, subfiles in os.walk(_directory):
...
def generate_indicator_data(metadata, row):
    from webportal.indicators.models import IndicatorData
    import datahub.indicators.conversion as conversion
    value = row[metadata['element_name']]
    key_field = get_key_field(metadata['file_name'])
    indicator_data_kwargs = {
...
def find_file(metadata):
    file_path = None
    for file, path in get_files().iteritems():
        if file.lower() == metadata['file_name'].lower() or file.lower() == metadata['file_name'].lower() + '.csv':
            file_path = path
    return file_path

def prep_indicator_definition(metadata):
    indicator = {}
        ...
        if file_path:
            ...
            for row in reader:
                if row.has_key(metadata['element_name']):
                    found_column = True
                    indicator_data = generate_indicator_data(metadata, row)
...
def create_indicator(indicator_def):
    return Indicator.objects.create(**indicator_def)

_indicator_list = None
def get_dynamic_indicator_def(metadata):
    global _indicator_list
    if not _indicator_list:
        _indicator_list = indicator_list()
    for indicator_pair in _indicator_list:
        if indicator_pair[0] == metadata['indicator_group'] + 'Indicator':
            return indicator_pair[1]

def insert_dynamic_data(indicator, metadata):
...
def insert_pregen_data(indicator, all_metadata):
    """ Find all rows for the indicator, and insert each """
    count = 0
    for metadata in [metadata for metadata in all_metadata if (
...
def new_run_all():
    ...
    _directory = os.path.abspath(os.path.join(
    ...
    for metadata in [metadata for metadata in all_metadata if metadata['indicator_group'] != '' and metadata['display_name'] != '']:
        indicator_def = prep_indicator_definition(metadata)
        try:
            ...
            indicator = create_indicator(indicator_def)
        seen_indicators.add(indicator)
        
        if metadata['hub_programming'].lower() == 'y':
            result = pool.apply_async(insert_dynamic_data, [indicator, metadata])
            #insert_dynamic_data(indicator, metadata)
        else:
            result = pool.apply_async(insert_pregen_data, [indicator, all_metadata])
            #insert_pregen_data(indicator)
